...
- rails and ruby, why these languages
...
The application also has a purchase history which allows the information from each purchase to be kept locally in the application database and then filtered according to which user is logged in.  A user cannot view another's purchases or purchase details due to the 
...
We kept a spreadsheet. Checking the spreadsheet we test the code regularly. Our Source control give error for any code that conflicts with master. After each GitHub merge another test was done. Finally we test the general application daily to figure out bugs and have users flow.
